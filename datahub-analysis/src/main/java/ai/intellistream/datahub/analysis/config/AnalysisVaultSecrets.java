 ...
 *
 * <p>This is the only Vault data this lean service needs; it has no DB, Pulsar, or session store.
 * Registered in {@code DatahubAnalysisApplication.main} (not {@code spring.factories}), so it runs on
 * real startup but not in {@code @SpringBootTest} contexts, which supply a dummy issuer instead.
 */
public final class AnalysisVaultSecrets implements VaultSecretContributor {
    ...
    public void contribute(Vault vault, VaultProperties properties, Map<String, Object> out) throws VaultException {
        ...
        var data = vault.logical().read(properties.secretName() + "/datahub-platform").getData();
        out.put("spring.security.oauth2.resourceserver.jwt.issuer-uri", data.get("keycloak.issuer"));
    }
}
