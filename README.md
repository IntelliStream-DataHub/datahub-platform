...
- **Configurable agents**: an agent is a named definition in your tenant's database — a prompt, a model backend, and an explicit list of the MCP tools it may use. The web console's chat panel runs one of them. Each tenant brings its own model: Anthropic, or any OpenAI-compatible/self-hosted server, configured in its own Vault entry, so one customer can run a hosted model while another runs an airgapped one.
- **Tool access governed from both ends**: what an agent can actually reach is its explicit allowlist intersected with the permissions of the identity it runs as — today the signed-in user, whose dataset grants come from `GET /tenant/permissions`. Neither half can widen the other: an allowlist cannot reach data the caller has no grant on, and a grant cannot reach a tool the allowlist omits.

Agents are read-only by design — the tool catalogue classifies every tool, and an allowlist naming a mutating one is rejected when it is saved, not when it is used. A direct MCP client sees the full tool set, but every call is authorised by the caller's own token, so scoping an agent is the same act as scoping any service account: grant its dataset groups read-only and it cannot write. Planned next: agents that run on a schedule or a trigger rather than only when someone asks, plus anomaly detection, data cleaning, and event detection built on the same graph-plus-signals foundation.
