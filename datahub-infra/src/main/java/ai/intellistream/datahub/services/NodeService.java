...
public class NodeService {
    ...
    public void mapCommonNodeFields(NodeEntity node, NodeModel form){
        ...
        // Timestamps are deliberately NOT copied from the body. They are declared READ_ONLY in the
        // schema and generated by Hibernate (@CreationTimestamp/@UpdateTimestamp), but the DTO
        // stamps now() in its field initialiser and its setter folds an explicit null back into
        // now() — so the old "copy it if non-null" was never a no-op and never could be skipped:
        // a create body could name any dateCreated it liked and have it stored. The adapter
        // services each tried to defend against that by setting null, which does nothing.
