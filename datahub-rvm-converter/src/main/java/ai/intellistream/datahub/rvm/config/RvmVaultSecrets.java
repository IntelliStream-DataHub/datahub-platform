 ...
 * store.
 * Registered in {@code DatahubRvmConverterApplication.main} (not {@code spring.factories}), so it
 * runs on real startup but not in {@code @SpringBootTest} contexts, which supply a dummy issuer
 * instead.
 */
public final class RvmVaultSecrets implements VaultSecretContributor {
