...
public class SubscriptionFanout {
     ...
     *
     * <p><b>Never blocks the caller.</b> This runs on the datapoints-listener pool, and building a
     * producer is a broker round-trip that can hang for the client's whole operation timeout (30s)
     * — a partitioned fanout topic with one partition over its backlog quota does exactly that,
     * refusing producer creation until an operator intervenes. Creating it synchronously under the
     * {@code fanoutProducers} monitor meant one listener thread burned that timeout holding the
     * lock while the rest of the pool queued behind it, so a stuck fanout stalled the ClickHouse
     * ingest it is supposed to be decoupled from. Fan-out is best-effort; waiting for it is not.
     */
